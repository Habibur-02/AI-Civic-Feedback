...
# Load API key
load_dotenv()
client = OpenAI(api_key=os.getenv("GPT_API_4_1"))
...
def render_submit_feedback(user: dict, notification_manager):
    ...
    # ---------------- 🎤 Voice Input Section ----------------
    st.markdown("#### 🎤 Optional: Record Your Feedback")
    audio = mic_recorder(
    ...
    if audio and "bytes" in audio:
        st.info("Audio recorded, sending to OpenAI Whisper API...")
        try:
            # send webm bytes directly
            bio = io.BytesIO(audio["bytes"])
            bio.name = "audio.webm"  # important for OpenAI API
            ...
            st.text_area("Transcribed Text", voice_text, height=100)
    ...
    with st.form("feedback_form", clear_on_submit=True):
        ...
        with col1:
            ...
            category = st.selectbox(
                "Category (Optional)",
                ["Auto-detect"] + ["Traffic", "Sanitation", "Safety", "Water",
        ...
        # 👇 Auto-fill description if voice_text exists
